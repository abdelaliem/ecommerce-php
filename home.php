...
<nav class="bg-white sticky top-0 w-full z-50 border-b border-gray-200">
    <div class="max-w-7xl mx-auto px-6 h-16 flex justify-between items-center font-sans text-sm font-medium tracking-tight">
        <a class="text-xl font-bold text-blue-700 hover:text-blue-700 transition-colors" href="home.php">AuraCommerce</a>
        ...
        <div class="flex items-center gap-4 text-blue-700">
            <a href="shopping-cart.php" class="relative hover:text-blue-900 transition-colors active:scale-95">
                ...
                <?php if ($cart_count > 0): ?>
                <span class="absolute -top-2 -right-2 bg-red-600 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-bold"><?= $cart_count ?></span>
                <?php endif; ?>
            </a>
            <?php if (isLoggedIn()): ?>
                <span class="text-sm text-gray-600">Hi, <strong><?= htmlspecialchars($_SESSION['user_name']) ?></strong></span>
                <?php if (isAdmin()): ?>
                    <a href="Admin/home.php" class="text-xs bg-blue-700 text-white px-3 py-1 rounded hover:bg-blue-800">Admin</a>
                <?php endif; ?>
    ...
    <section class="w-full max-w-[1200px] mx-auto px-6 py-section-padding flex flex-col md:flex-row items-center gap-stack-lg border-b border-surface-variant">
        <div class="flex-1 flex flex-col items-start gap-stack-md">
            ...
            <div class="flex gap-4 pt-4">
                <a class="bg-primary text-on-primary font-label-sm text-label-sm px-8 py-4 rounded-DEFAULT hover:bg-on-primary-fixed-variant transition-colors" href="#products">
                    Shop Collection
                </a>
            </div>
        </div>
        <div class="flex-1 w-full relative">
            <div class="aspect-[4/3] bg-surface-container-low rounded-lg overflow-hidden border border-outline-variant">
    ...
    <section id="products" class="w-full max-w-[1200px] mx-auto px-6 py-section-padding flex flex-col gap-stack-lg">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="group flex flex-col gap-3">
                <a href="product.php?id=<?= $p['id'] ?>" class="block aspect-square bg-surface-container-low rounded-DEFAULT border border-surface-variant overflow-hidden relative">
                    ...
                    <?php if ($p['stock'] <= 0): ?>
                        <div class="absolute top-2 left-2 bg-red-600 text-white px-2 py-1 font-label-sm text-xs rounded-sm">OUT OF STOCK</div>
                    <?php elseif ($p['stock'] < 5): ?>
                        <div class="absolute top-2 left-2 bg-orange-500 text-white px-2 py-1 font-label-sm text-xs rounded-sm">LOW STOCK</div>
                    <?php else: ?>
                        <div class="absolute top-2 left-2 bg-primary text-on-primary px-2 py-1 font-label-sm text-xs rounded-sm"><?= htmlspecialchars($p['category'] ?? 'NEW') ?></div>
                    <?php endif; ?>
                </a>
                <div class="flex flex-col">
                    <a href="product.php?id=<?= $p['id'] ?>" class="font-label-sm text-label-sm text-on-surface hover:text-primary truncate"><?= htmlspecialchars($p['name']) ?></a>
                    <p class="font-body-md text-body-md text-secondary mt-1">$<?= number_format($p['price'], 2) ?></p>
