...
<style>
    ...
    .page-subtitle { color: #6b7280; font-size: 0.95rem; margin-top: 0.35rem; }
    .btn-report { background: #2563eb; color: #fff; border: none; padding: 0.55rem 1.25rem; border-radius: 8px; font-size: 0.875rem; font-weight: 600; display: flex; align-items: center; gap: 0.4rem; transition: background 0.2s; cursor: pointer; text-decoration: none; }
    .btn-report:hover { background: #1d4ed8; color: #fff; }

    /* Stat Cards */
    .stats-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 1.25rem; margin-bottom: 2rem; }
    .stat-card { background: #fff; border: 1px solid #e5e7eb; border-radius: 12px; padding: 1.5rem; display: flex; align-items: flex-start; gap: 1rem; transition: box-shadow 0.2s; }
    ...
    .stat-label { font-size: 0.8rem; font-weight: 500; color: #6b7280; text-transform: uppercase; letter-spacing: 0.04em; margin-bottom: 0.35rem; }
    .stat-value { font-size: 1.65rem; font-weight: 700; color: #111827; line-height: 1.2; }
    .stat-trend { font-size: 0.75rem; margin-top: 0.35rem; display: flex; align-items: center; gap: 0.25rem; }
    .stat-trend.up { color: #059669; }
    .stat-trend.down { color: #dc2626; }

    /* Cards */
    .card-section { background: #fff; border: 1px solid #e5e7eb; border-radius: 12px; padding: 1.5rem; margin-bottom: 1.5rem; }
    .card-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.25rem; }
    .card-title { font-size: 1.05rem; font-weight: 700; color: #111827; }
    .card-link { font-size: 0.8rem; color: #2563eb; text-decoration: none; font-weight: 500; display: flex; align-items: center; gap: 0.25rem; }
    .card-link:hover { text-decoration: underline; }

    /* Orders Table */
    .orders-table { width: 100%; border-collapse: collapse; }
    .orders-table thead th { padding: 0.6rem 0.75rem; font-size: 0.7rem; font-weight: 600; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; border-bottom: 1px solid #e5e7eb; text-align: left; }
    .orders-table tbody td { padding: 0.75rem; font-size: 0.875rem; color: #374151; border-bottom: 1px solid #f3f4f6; vertical-align: middle; }
    .orders-table tbody tr:last-child td { border-bottom: none; }
    .orders-table tbody tr:hover { background: #f9fafb; }
    .status-badge { padding: 0.2rem 0.65rem; border-radius: 20px; font-size: 0.72rem; font-weight: 600; display: inline-block; }
    ...
    /* Product List */
    .product-item { display: flex; align-items: center; gap: 0.85rem; padding: 0.65rem 0; border-bottom: 1px solid #f3f4f6; }
    .product-item:last-child { border-bottom: none; }
    .product-img { width: 44px; height: 44px; border-radius: 8px; object-fit: cover; background: #f3f4f6; flex-shrink: 0; }
    .product-info { flex: 1; }
    .product-name { font-size: 0.875rem; font-weight: 600; color: #111827; }
    .product-cat { font-size: 0.75rem; color: #6b7280; }
    .product-sales { font-size: 0.875rem; font-weight: 600; color: #111827; text-align: right; }

    /* Customer List */
    .customer-item { display: flex; align-items: center; gap: 0.75rem; padding: 0.65rem 0; border-bottom: 1px solid #f3f4f6; }
    .customer-item:last-child { border-bottom: none; }
    .customer-avatar { width: 38px; height: 38px; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #fff; font-weight: 600; font-size: 0.75rem; flex-shrink: 0; }
    .customer-info { flex: 1; }
    .customer-name { font-size: 0.875rem; font-weight: 600; color: #111827; }
    .customer-email { font-size: 0.75rem; color: #6b7280; }
    .customer-date { font-size: 0.75rem; color: #9ca3af; text-align: right; white-space: nowrap; }

    .empty-note { text-align: center; color: #9ca3af; padding: 2rem; font-size: 0.875rem; }
</style>
    ...
    <div class="card-section">
        ...
        <table class="orders-table">
            ...
            <?php while ($order = $recentOrders->fetch_assoc()): ?>
                <tr>
                    <td style="font-weight:600; color:#2563eb;">#ORD-<?= str_pad($order['id'], 4, '0', STR_PAD_LEFT) ?></td>
        ...
        <?php else: ?>
        <div class="empty-note"><i class="bi bi-inbox" style="font-size:2rem; display:block; margin-bottom:0.5rem;"></i>No orders yet.</div>
        <?php endif; ?>
    </div>
        ...
        <div class="card-section">
            ...
            <?php if ($topProducts && $topProducts->num_rows > 0): ?>
                ...
                <div class="product-item">
                    ...
                    <?php else: ?>
                        <div class="product-img" style="display:flex;align-items:center;justify-content:center;color:#9ca3af;"><i class="bi bi-box"></i></div>
                    <?php endif; ?>
                    <div class="product-info">
                        <div class="product-name"><?= htmlspecialchars($prod['name']) ?></div>
                        <div class="product-cat"><?= htmlspecialchars($prod['category'] ?? 'Uncategorized') ?></div>
                    </div>
                    <div class="product-sales"><?= $prod['sold'] ?> sold</div>
                </div>
                <?php endwhile; ?>
            <?php else: ?>
        ...
        <div class="card-section">
            ...
            <?php if ($recentCustomers && $recentCustomers->num_rows > 0): ?>
                ...
                <div class="customer-item">
                    <div class="customer-avatar" style="background:<?= avatarColor($cust['name']) ?>"><?= initials($cust['name']) ?></div>
                    <div class="customer-info">
                        <div class="customer-name"><?= htmlspecialchars($cust['name']) ?></div>
                        <div class="customer-email"><?= htmlspecialchars($cust['email']) ?></div>
                    </div>
                    <div class="customer-date"><?= date('M d, Y', strtotime($cust['created_at'])) ?></div>
                </div>
                <?php endwhile; ?>
            <?php else: ?>
