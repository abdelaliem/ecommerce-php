...
<div style="flex: 1; padding: 30px 35px; background-color: #f8f9fa; min-height: 100vh;">
    ...
    <div class="d-flex justify-content-between align-items-start mb-4">
        <div>
            ...
            <p style="color: #6b7280; font-size: 0.95rem; margin: 0;">Manage your product catalog, pricing, and current
                stock levels.</p>
        </div>
        <button class="btn d-flex align-items-center gap-2" data-bs-toggle="modal" data-bs-target="#addModal"
            style="padding: 10px 20px; border-radius: 8px; font-weight: 500; background-color: #1e3a5f; color: #fff;">
        ...
        <div class="card-body p-4">
                ...
                <div class="d-flex" style="max-width: 400px; flex: 1;">
                    <div class="input-group" style="border: 1px solid #d1d5db; border-radius: 8px; overflow: hidden;">
                        ...
                        <input type="text" name="search" class="form-control border-0 shadow-none"
                            placeholder="Search by name, SKU..." value="<?= htmlspecialchars($search) ?>"
                ...
                <div class="d-flex gap-2">
                    <div class="dropdown">
                        <button class="btn btn-outline-secondary d-flex align-items-center gap-2 dropdown-toggle"
                            ...
                            style="border-radius: 8px; font-size: 0.9rem; border-color: #d1d5db;">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end"
                            ...
                            <li><a class="dropdown-item <?= $filter_category === '' ? 'active' : '' ?>"
                                    href="?search=<?= urlencode($search) ?>">All Categories</a></li>
                            <li>
                                <hr class="dropdown-divider">
                            </li>
                            <?php foreach ($categories as $cat): ?>
                                <li><a class="dropdown-item <?= $filter_category === $cat ? 'active' : '' ?>"
                                        href="?category=<?= urlencode($cat) ?>&search=<?= urlencode($search) ?>">
                                ...
                                <tr style="border-bottom: 1px solid #f3f4f6;">
                                    <td style="padding: 16px;">
                                        <div class="d-flex align-items-center gap-3">
                                            <div
                                                style="width: 48px; height: 48px; border-radius: 8px; overflow: hidden; background: #f3f4f6; flex-shrink: 0;">
                                    ...
                                    <td style="padding: 16px;">
                                        <span class="stock-badge <?= $badge_class ?>">
                                            <span
                                                style="display: inline-block; width: 7px; height: 7px; border-radius: 50%; background: <?= $dot_color ?>; margin-right: 6px;"></span>
                                            <?= $badge_text ?>
                                        </span>
                                    </td>
                                    <td style="padding: 16px;">
                                        <div class="d-flex gap-1">
                                            <button class="btn btn-sm btn-light edit-btn" style="border-radius: 6px;"
                                            ...
                                            <a href="inventory.php?delete=<?= $product['id'] ?>" class="btn btn-sm btn-light"
            ...
            <?php if ($total_pages > 1): ?>
                <div class="d-flex justify-content-between align-items-center mt-4 pt-3"
                    ...
                    <span style="color: #6b7280; font-size: 0.85rem;">Showing
                        <?= $offset + 1 ?> to
                        <?= min($offset + $per_page, $total_products) ?> of
                    ...
                    <nav>
                        <ul class="pagination pagination-sm mb-0 gap-1">
                            ...
                            <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                                <li class="page-item">
                                    <a class="page-link custom-page-link <?= ($i == $page) ? 'active-page' : '' ?>"
        ...
        <div class="modal-content" style="border: none; border-radius: 12px;">
            ...
            <form method="POST">
                ...
                <div class="modal-body pt-3">
                    <div class="mb-3">
                        <label class="form-label fw-semibold" style="color: #374151; font-size: 0.9rem;">Product Name
                            <span class="text-danger">*</span></label>
                        <input type="text" name="name" class="form-control custom-input"
                            placeholder="e.g. Elite Wireless Headphones" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold"
                            style="color: #374151; font-size: 0.9rem;">Description</label>
                        <textarea name="description" class="form-control custom-input" rows="2"
                            placeholder="Short description..."></textarea>
                    </div>
                    <div class="row mb-3">
                        <div class="col-6">
                            <label class="form-label fw-semibold" style="color: #374151; font-size: 0.9rem;">Price ($)
                                <span class="text-danger">*</span></label>
                            <input type="number" name="price" class="form-control custom-input" step="0.01" min="0"
                                placeholder="0.00" required>
                        </div>
                        <div class="col-6">
                            <label class="form-label fw-semibold" style="color: #374151; font-size: 0.9rem;">Stock <span
                                    class="text-danger">*</span></label>
                            <input type="number" name="stock" class="form-control custom-input" min="0" placeholder="0"
                                required>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold"
                            style="color: #374151; font-size: 0.9rem;">Category</label>
                        <input type="text" name="category" class="form-control custom-input"
                            placeholder="e.g. Electronics">
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold" style="color: #374151; font-size: 0.9rem;">Image
                            URL</label>
                        <input type="url" name="image" class="form-control custom-input"
                            placeholder="https://example.com/image.jpg">
                    </div>
                </div>
                <div class="modal-footer border-0 pt-0">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal"
                        style="border-radius: 8px; border-color: #d1d5db;">Cancel</button>
                    <button type="submit" class="btn"
                        style="background-color: #1e3a5f; color: #fff; border-radius: 8px;">
            ...
            <form method="POST">
                ...
                <div class="modal-body pt-3">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-semibold"
                            style="color: #374151; font-size: 0.9rem;">Description</label>
                        <textarea name="description" id="edit-description" class="form-control custom-input"
                            rows="2"></textarea>
                    </div>
                    <div class="row mb-3">
                        <div class="col-6">
                            <label class="form-label fw-semibold" style="color: #374151; font-size: 0.9rem;">Price ($)
                                <span class="text-danger">*</span></label>
                            <input type="number" name="price" id="edit-price" class="form-control custom-input"
                                step="0.01" min="0" required>
                        </div>
                        <div class="col-6">
                            <label class="form-label fw-semibold" style="color: #374151; font-size: 0.9rem;">Stock <span
                                    class="text-danger">*</span></label>
                            <input type="number" name="stock" id="edit-stock" class="form-control custom-input" min="0"
                                required>
                        </div>
                    </div>
                    <div class="mb-3">
